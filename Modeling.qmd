...
LR1_fit <- LR1_wkf |>
  fit_resamples(diab_folds, metrics = mn_log_loss())
```
...
```{r}
LR3_rec <- recipe(Diabetes_binary ~ Age + BMI + HighBP + Age:BMI + Age:HighBP, 
                  data = train) |>
  step_normalize(all_numeric()) |>
  step_dummy(all_nominal())
...
```
As we can see from the log loss metrics, our best model is ___.
...
tree_best_params <- select_best(tree_fits, metric = "log_loss")
...
rf_final_fit <- rf_final_wkf |>
  last_fit(diab_split, metrics = mn_log_loss)
```
