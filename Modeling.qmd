...
```{r}
log_fit <- fit(LR3_wkf, data = train)
class_fit <- fit(tree_final_wkf, data = train)
rand_fit <- fit(rf_final_wkf, data = train)

log_preds <- predict(log_fit, test)
class_preds <- predict(class_fit, test)
rand_preds <- predict(rand_fit, test)
```
